...
import { AshtaRecord, AshtaSiddhantaService } from '../services/ashta-siddhanta.service';
...
@Component({
  selector: 'app-home',
  standalone: true,
  imports: [CommonModule, IonicModule, FormsModule, NakshatraModule, PanchangaModule],
...
export class HomePage implements OnInit {
  ...
  selectedTab: string = 'current';
    ...
    ngOnInit() {
      ...
      this.ashtaService.getData().subscribe((data) => {
        ...
        if (!this.currentSunrise) {
          ...
          this.offsetMinutesCurrent = this.computeOffset(this.currentSunrise, '06:00:00');
        }
        // Build segments for current cycle and for daily table (default: today)
      ...
      this.http.get<any[]>('assets/static-data/geodata.min.json').subscribe(
        (data) => {
          this.cities = data;
          // Optionally, initialize filteredLocations with all cities
    ...
    doRefresh(event?: any) {
      ...
      this.getUserLocation();
      // If a refresher event is provided, complete it.
      if (event) {
    ...
    async getSunriseForLocation(location: any) {
      ...
      this.offsetMinutesCurrent = this.computeOffset(this.currentSunrise, '06:00:00');
      this.loadDailySegments(new Date(this.selectedDate), lat, lng, location.timezone);
    }
    ...
    loadCurrentDaySegments() {
      ...
      const segments12h = this.rawData.filter(rec => rec.DayOfWeek === cycleWeekday);
      ...
      const secondHalf = segments12h.map(rec => ({
        adjustedStart: this.addOffset(rec.StartTime, this.offsetMinutesCurrent + 720),
     ...
     */
    loadDailySegments(chosenDate: Date, lat?: number, lng?: number, timezone?: string) {
      if (!this.rawData.length) { return; }
      if (!lat || !lng) {
    ...
    getLatLng(): { lat: number, lng: number } {
      if (this.locationString !== 'Unknown' && this.locationString.indexOf('Lat:') > -1) {
    ...
    computeCurrentSegment() {
      if (!this.adjustedSegmentsCurrent || this.adjustedSegmentsCurrent.length === 0) {
  ...
  convertToTimezone(dateStr: string, toTimezone: string): {timeOnly: string, weekday: string} {
    try {
      // Parse the input date
      const inputDate = moment.tz(dateStr, "ddd MMM DD YYYY HH:mm:ss [GMT]ZZ", "Etc/GMT");
