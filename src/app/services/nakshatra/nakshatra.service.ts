...
export class NakshatraService {
  ...
  private jd(d: number, m: number, y: number) { 
      var a, j, l; 
      var b; 
      if (m < 3) { 
          m += 12; 
  ...
  private calculateB6(d: any, m: any, y: any) { 
  ...
  private lahiriAyan(dd: any, mm: any, yy: any) { 
  ...
  private computeMoonEclipticLongitude(date: Date): number {
  ...
  private computeSiderealLongitude(date: Date): number {
  ...
  private binarySearchSidereal(target: number, startDate: Date, endDate: Date): Date {
  ...
  public determineNakshatra(date: Date): string {
  ...
  public determineNakshatraStartEnd(date: Date): Panchanga {
    const dt = new Date(date);
    const siderealLongitude = this.computeSiderealLongitude(date);
  ...
  public determineNextNakshatra(date: Date): Nakshatra {
    const dt = new Date(date);
    const siderealLongitude = this.computeSiderealLongitude(date);
    const interval = 360 / 27; 
    const nakshatraIndex = Math.floor(siderealLongitude / interval);
  ...
  public getAllNakshatras(): string[] {
